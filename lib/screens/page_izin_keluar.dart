...
import 'package:hahai/screens/input_izin_keluar.dart';
...
class PageIzinKeluar extends StatefulWidget {
  const PageIzinKeluar({Key? key}) : super(key: key);

  @override
  State<PageIzinKeluar> createState() => _PageIzinKeluarState();
}

//enum for rambu kondisi
class _PageIzinKeluarState extends State<PageIzinKeluar> {
  ...
  Future<void> getIzin() async {
    ...
    final data = await dioProvider.getIzin(); // Use the initialized DioProvider
    if (data != 'Error') {
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Pengajuan Izin Keluar'),
      ),
        ...
        child: Column(
          ...
          children: <Widget>[
            FloatingActionButton(
                ...
                heroTag: null,
                onPressed: ()async{
                  String refresh = await Navigator.push(context, MaterialPageRoute(builder: (context)=> InputIzinKeluar()));
            ...
            child: RefreshIndicator(
              ...
              artikel.isNotEmpty
                    ...
                    itemBuilder: (context, index) {
                        ...
                        onTap: () {
                          ...
                          showDialog(
                            ...
                            builder: (BuildContext context) {
                              return AlertDialog(
                                title: Text('Detail Persetujuan Izin'),
                                content: SingleChildScrollView(
                                  child: Container(
                                    ...
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: article['persetujuan_izin']
                                          .map<Widget>((approval) {
                                        return Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          children: [
                                ...
                                actions: <Widget>[
                                  if (article['persetujuan_izin'].isEmpty )
                                    ElevatedButton(
                                      onPressed: () {
                                        _onDeleteClick({
                                          'izin_id': article['id'],
                      ...
                      : Center(child: Text('Belum Ada Pengajuan Izin Keluar')),
              ),
                ],
              ),
            ),
  ...
  void _onDeleteClick(Map<String, dynamic> data) async {
    try {
      ...
      final response = await Dio().delete(
        '$url/api/izin/${data['izin_id']}', // Use DELETE method and include the izin_id in the URL
        data: formData,
        options: Options(
          headers: {
