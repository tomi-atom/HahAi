...
class PageLemburApprove extends StatefulWidget {
  const PageLemburApprove({Key? key}) : super(key: key);

  @override
  State<PageLemburApprove> createState() => _PageLemburApproveState();
}

//enum for rambu kondisi
class _PageLemburApproveState extends State<PageLemburApprove> {
  ...
  Future<void> getApproveLembur() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
    final data = await dioProvider.getApproveLembur(); // Use the initialized DioProvider
    if (data != 'Error') {
      setState(() {
              ...
              child:
                  artikel.isNotEmpty
                    ...
                    itemBuilder: (context, index) {
                      ...
                      : Center(child: Text('Belum Ada Pengajuan Lembur yang Perlu disetujui')),
              ),
                ],
              ),
            ),
